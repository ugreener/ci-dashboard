...
tracking:
  blocklist:
    - "[FAR] Verify FenceAgentsRemediation CR remediation flow"
    - "[SBR] Some flaky test description to exclude"
```

Blocklist entries match the `test_name` stored in the database. For medik8s Ginkgo tests, this is the cleaned test description with operator tags preserved (e.g., `[FAR]`, `[SBR]`, `[SNR]`). Ginkgo execution decorators like `[Serial]` and `[Slow]` are stripped automatically, but operator identity tags must be included in blocklist entries for exact matches to succeed.
